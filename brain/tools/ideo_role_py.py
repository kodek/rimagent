...
def ideo_role(ctx, pawn, action="list", role=None, force=False):
    ...
    # Build role list - roles may come back as dicts or strings
    role_list = []
    for i, r in enumerate(roles):
        if isinstance(r, dict):
            role_list.append({
                ...
                "label": r.get("label") or r.get("defName") or str(r),
            })
        else:
            # string form: "IdeoRole_Priest" or similar
            role_list.append({"index": i, "defName": str(r), "label": str(r)})
    ...
    if action == "assign":
        ...
        for i, r in enumerate(roles):
            ...
            if role_lower in name:
                ...
                break
        if target is None:
            available = [r.get("defName") if isinstance(r, dict) else str(r) for r in roles]
