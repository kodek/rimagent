...
- **Drafted pawns cannot put out fires.** The `fire` standing order only raises Firefighter for an hour (posture `firefight`) and logs the fire; it never drafts or undrafts. If the combat order has the fighters drafted when a fire starts inside the walls, undraft two yourself (`rw_ui_draft(drafted=false)`; they stay hands-off to the combat order for an hour, which is what you want).
...
- **Dumping zone:** create a `DumpingStockpile` preset zone outside the home area for corpses, rotting food, and filth. Keep it off walking paths. `rw_ui_zone(action=create_stockpile, preset=DumpingStockpile, rect=..., label="dump")`. The `corpses` order designates human corpses for a grave/sarcophagus when one has space, otherwise strips and hauls them to a stockpile labelled `corpses` if one exists, otherwise burns them (crematorium bill or campfire burn); rotten corpses go to the dump. Build a few graves (`Grave`, 1x2) early so it has somewhere to put raiders; a stockpile named `corpses` outside the walls is the cheap alternative.

## Bedroom vs barracks
- The `beds` order assigns every free colonist an owned bed when a free one exists (lovers share a double bed if one stands, the room nearest their last bed preferred) and prisoners a prisoner bed; a bed you assigned by pressing its gizmo is left alone for 2 days. Intervene only for a pairing it cannot know (a feud, a night owl in a shared room). Bedroom = only the owner's bed(s) (a lover pair is fine), no medical/prison bed; more than one unassigned bed makes a barracks. A barracks costs about **-5 mood vs an equivalent private bedroom** (-4 at higher quality). One barracks is fine on day 1; split into bedrooms within the first season.
...
4. **Bedroom**: 5x5 interior minimum, one bed, an end table + dresser + recreation (horseshoes pin / darktorch / etc) to lift impressiveness and joy. The `beds` order assigns the owner once the bed is built (clears the barracks -7); press the bed's assign gizmo only to force a particular pairing.
...
8. **Order of build**: roofed storage + bedroom first (mood + anti-degradation), then kitchen, then research room. Queue all blueprints, posture `build` if they must go up before anything else, and let it flow.
9. **After the build:** run the sealed-room check above on every new room. Verify doors are placed and rooms are reachable.

## Blueprint hygiene
The `blueprints` standing order cancels, every 2500 ticks, blueprints and frames no colonist can reach or that sit on a cell nobody can build on (thick roof, deep water), and its summary reports how many it cancelled and which blueprints are older than 3 days with materials missing (no action on those). So a blueprint that vanished was unreachable: check the door and the wall line before placing it again. A blueprint that stays is starved or outscored: `blueprint_check` for the material, `rw_steward_explain(work="Construction")` for the builder, posture `build` for 12 h. Do not queue what you cannot pay for; a 3-day-old starved blueprint is wealth-free clutter that the order will keep nagging about in `rw_steward_orders`.
...
3. Kitchen: raw-food shelf/stockpile **adjacent** to the stove (otherwise the cook hauls one meal's ingredients per trip); meal stockpile next door, later a freezer (coolers blue side in, fully enclosed and roofed, usually 2+). Keep fields, kitchen and freezer within a short walk. A small room with two beds next to the kitchen becomes the hospital: give the room the Hospital role (or name it `hospital`) and the `beds` order marks its beds medical; put the rally point next to it.
