...
- **Never guess a defName for something you haven't built this game.** "Table"/"Chair" are not real defs (real ones: `Table1x2c`/`Table2x2c`/`Table3x3c`, `DiningChair`/`Armchair`) and every wrong guess wastes a call. Call `rw_defs_buildable` or `rw_defs_search` first; the same goes for recipe names on `rw_ui_add_bill` and workbench ids (pass the built thing's id as `thing`, not the blueprint's).
- **The same goes for RPC method names when using `rpc(method, **params)` in `run_python`.** There is no `ledger.*` method family (or any other you haven't seen in a doc string); recent ledger events already arrive in your situation packet automatically, you never need to pull them. If you want to call something by raw method name and you are not certain it exists, check `rw_bridge_methods` first rather than guessing two or three plausible-sounding names in a row.
...
4. **Blueprints: `rw_ui_build`.** `def` (ThingDef or TerrainDef), then one of `at=[x,z]`, `line=[[x1,z1],[x2,z2]]`, `rect=[x,z,w,h]` (+`fill=true` for a filled area, default outline). `rot=N|E|S|W` for beds/tables/doors when orientation matters. `stuff` is REQUIRED for stuff-made things (Wall/Door/Bed/etc) — nothing is auto-picked; omit it once to get the options with on-map quantities, then pass e.g. `stuff="BlocksGranite"`. Pass `dry_run=true` first for big placements: it returns `placed`, `failed` (cell + reason), `cost_each` and `work`. Floors are TerrainDefs (e.g. `WoodPlankFloor`), placed the same way.
...
7. **`run_python` is a persistent REPL**: `beds = find("Bed")["things"]` stays available next step; `base()`, `summary()`, `detail(...)`, `build(...)`, `rpc(method, **params)` are pre-bound. **`find`/`build` take the defName as the first positional argument, not `def=`** — `def` is a Python reserved word and `find(def="Bed")` is always a `SyntaxError`, never a bridge error; other filters are still keyword args, e.g. `find("Bed", near=[10,10])`. `rpc("map.find", **{"def": "Bed"})` also works if you need the raw method name. Compute, don't guess.
