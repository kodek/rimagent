...
2. **The colony as a whole undervalues something** (nobody researches for days): `rw_steward_settings(scorer={"globalWorkAdjustments":{"Research":0.4}})`. This writes the RimBridge mod settings file: it survives new games, episodes and restarts (the notebook does not), so prefer a posture for anything per-colony, journal it if you keep it, and reset it with `rw_steward_settings(scorer={"globalWorkAdjustments":{"Research":null}})` (scalar fields must be set back to their default value explicitly).
