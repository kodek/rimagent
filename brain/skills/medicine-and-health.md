...
- Medical policy: the `policies` standing order sets colonists to NormalOrWorse and prisoners and animals to HerbalOrWorse, and resets to that after 2 days if you set a pawn's care by hand (or it changed outside the order): set it back yourself when the plague or infection is over, or you keep burning good medicine. Intervene per patient: `rw_ui_set_policies(pawn=, medical=Best)` the moment infection or plague appears, `HerbalOrWorse` on a colonist with bruises while industrial medicine is scarce, `NoMeds` on a prisoner you will not recruit.
