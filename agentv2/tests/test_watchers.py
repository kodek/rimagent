...
from agentv2.watchers import Watcher, WatcherError, Watchers
...
def write(watchers: Watchers, name: str, source: str) -> None:
    (watchers.directory / f"{name}.py").write_text(source, encoding="utf-8")
...
async def test_alert_and_action(watchers, game, bus):
    ...
    assert alerts == [{"watcher": "raid", "text": "RAID", "wake": True}]
...
async def test_rpc_is_read_only(watchers):
    ...
    await watchers.run_all([], {})
    assert "PermissionError" in watchers.loaded["bad"].error


async def test_sandbox_blocks_the_host_and_runaway_loops(watchers):
    with pytest.raises(WatcherError, match="PermissionError"):
        await watchers.evaluate(Watcher("fs", "import os\ndef watch(e, s, m):\n    return os.listdir('/')\n", "d"), [], {}, {})
    with pytest.raises(WatcherError, match="time limit"):
        await watchers.evaluate(Watcher("loop", "def watch(e, s, m):\n    while True:\n        pass\n", "d"), [], {}, {})


async def test_failed_watcher_is_disabled_until_its_file_changes(watchers, bus):
    write(watchers, "w", "def watch(events, status, memo):\n    return 1 / 0\n")
    await watchers.run_all([], {})
    assert "ZeroDivisionError" in watchers.loaded["w"].error
    assert watchers.errors() == {"watcher w": watchers.loaded["w"].error}
    write(watchers, "w", "def watch(events, status, memo):\n    return []\n")
    await watchers.run_all([], {})
    assert watchers.loaded["w"].error is None


async def test_wrong_return_type(watchers):
    with pytest.raises(WatcherError, match="list of dicts"):
        await watchers.evaluate(Watcher("x", "def watch(e, s, m):\n    return ['no']\n", "d"), [], {}, {})
